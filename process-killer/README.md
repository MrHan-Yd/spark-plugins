...
在 Spark 主输入框输 `kill <关键词>`(或关键字「进程终结者」/「终结者」)回车,带着关键词直接开窗搜索,例如:

- `kill 8080` → 打开即搜 8080
- 页面内:`:8080` 端口占用;`pid:1234` 指定 PID;`port 3000` / `端口 3000`;纯数字 = PID 或端口双查;多个关键词空格分隔(且关系);普通文本按名称/路径子串匹配

**模糊匹配**:精确命中之外自动带模糊候选(分数排后、行内带标签区分)——查端口 `11111` 输 `11`、`111` 即出前缀候选,越输越收窄;`pid:` 与 `:端口` 同样支持前缀;进程名/路径缺字也能命中(子序列模糊,如 `chrme` → chrome)。
...
- 关键字:`kill` / `进程终结者` / `终结者`;开发阶段只在 `0.1.0/` 迭代。