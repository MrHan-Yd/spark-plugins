...
### 5.9 agent 回查通道（recall）

`trace-recall` 的定位从「给人看的监督面」放大为「**agent 的经验记忆**」：`.agents/runner/recall.mjs`
是给 agent 自己的只读查询面，直接消费 `.agents/trace/` 的既有落盘（events.jsonl / index.json 的
notes），零新增数据格式。提示词条款在 `AGENTS.md §5`（动手前先查），查询语义在本节固定：

| 查询 | 语义 | 口径 |
|---|---|---|
| `--path <相对路径>` | 该文件的完整历史 | 命中写盘（`fs/write_applied`）、审批被拒（`approval/decision approved=false`，目标从 parent 的 params 解出）、执行被拦（`-32001`，同上）、合规未过（FAILED/ERROR）；按 ts 倒序。**正文包含该路径的决策笔记一并给出** —— 路径的历史决策往往就在笔记里 |
| `--rule <rule_id>` | 该检查器的台账 | total/passed/failed/skipped/error 五桶 + 未通过条目（对象从 parent 解出）；正文提到该规则的笔记一并给出 |
| `--denied` | 全库被拦/被拒 | 按「kind + 目标 rel」聚合，附**会话数**（与 §7.3 避坑智库的会话去重口径一致）；rejected 笔记清单附带 |
| `--notes` | 全库决策笔记清单 | id/status/category/title/file |
| `--json` | 机器可读输出 | 与文本输出同一份数据结构，agent 优先用这个 |

不变量：**永远 exit 0**（连未知参数都回落 usage/notes）——回查是查询不是闸门，任何失败只如实
说明，不阻断干活中的 agent；这与 hook-recorder 的红线（exit 0、静默）同一条哲学。路径匹配按
「剥掉 `.agents/trace/<会话>/` 前缀 + 双向 endswith」归一，容忍查询时带不带会话前缀。契约测试
`.agents/runner/tests/recall-test.mjs` 铺合成样本断言三类查询（10 项）。
...
| **文件档案** | 这个文件为什么变成现在这样 | 按文件聚合写盘/被拦/合规（digest 三路），有被拦或合规未过的排前；展开看单文件全部运行事实，行内点会话号跳详情 | 同左（纯 trace 视图，天然无笔记分叉） |
...
- **锚点深链**：笔记正文里的 `@trace s_xxx#N` 渲染成可点 chip —— 先查已载入索引，miss 时按锚点
  id 探 `<trace根>/<sid>/events.jsonl`（dir/fs 源可行；http/inline 探不了就明说，不造假按钮）；
  跳转后目标事件行高亮并自动展开原始报文。决策笔记 → 具体事件 → diff 的链路由此焊死。
- 顶栏**全局搜索**（`/` 聚焦）按标题+id+正文过滤笔记类视图；`refd_by` 由宿主构建期算好（§5.8）。
- 「被否决清单」从 v2 的建议项落成两个实体：**rejected 笔记**（人的取舍）与**运行时拦截**（协议事实）。
- 会话详情**自动刷新**：进入详情后 3s 轮询 events.jsonl 文本长度探针，变长才重读重渲染；
  不变量「`session.events` 非空 ⇔ 内联死数据」；切会话/切模块/换源/页面隐藏四处收口
  （决策笔记：`.agents/notes/implemented/feature/2026-09-16-会话详情自动刷新.md`）。
